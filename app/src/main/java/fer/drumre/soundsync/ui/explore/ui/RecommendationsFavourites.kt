...
@Composable
fun RecommendationsFavourites(
    recommendationsFavoritesUiState: RecommendationsFavoritesUiState,
    favoritesUiState: FavouritesUiState,
    onFavouriteClick: (Favourite) -> Unit,
) {
    val list = recommendationsFavoritesUiState.recommendationsFavorites.entries.map {
        Recommendation(
            artistName = it.key,
            tracks = it.value,
        )
    ...
    if (list.isNotEmpty()) {
        Column {
            Text(
                text = stringResource(R.string.based_on_your_favourites),
                color = AppSecondary,
